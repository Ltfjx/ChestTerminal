...
public abstract class QuartzDrill extends AContainer {
	...
	public QuartzDrill(Category category, ItemStack item, String name, RecipeType recipeType, ItemStack[] recipe) {
		...
		new BlockMenuPreset(name, getInventoryTitle()) {
			...
			public boolean canOpen(Block b, Player p) {
				...
				if (!OreGenSystem.wasResourceGenerated(OreGenSystem.getResource("乳白色石英"), b.getChunk())) {
	...
	protected void tick(Block b) {
		...
		else if (OreGenSystem.getSupplies(OreGenSystem.getResource("乳白色石英"), b.getChunk(), false) > 0) {
			MachineRecipe r = new MachineRecipe(24, new ItemStack[0], new ItemStack[] {ChestTerminal.quartz});
			if (!fits(b, r.getOutput())) return;
			processing.put(b, r);
			progress.put(b, r.getTicks());
			OreGenSystem.setSupplies(OreGenSystem.getResource("乳白色石英"), b.getChunk(), OreGenSystem.getSupplies(OreGenSystem.getResource("乳白色石英"), b.getChunk(), false) - 1);
		}
	}
